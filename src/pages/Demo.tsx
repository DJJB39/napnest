    ...
    <div className="min-h-[100dvh] pb-24">
      ...
      <div className="bg-primary/10 border-b border-border px-4 py-3 flex items-center justify-between">
        <p className="text-xs text-muted-foreground">
          <span className="font-semibold text-foreground">Demo Mode</span> — This is sample data for {DEMO_CHILD_NAME}
        </p>
        <Button size="sm" className="rounded-xl text-xs h-7" onClick={() => navigate("/auth")}>
          Sign Up Free
        </Button>
      </div>
              ...
              <CardContent className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={demoThisWeek}>
                    <RXAxis dataKey="day" tick={{ fill: "hsl(215 20% 65%)", fontSize: 11 }} axisLine={false} tickLine={false} />
                    <RYAxis tick={{ fill: "hsl(215 20% 65%)", fontSize: 11 }} axisLine={false} tickLine={false} unit="h" width={30} />
              ...
              <CardContent className="h-48">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={nhsChartData}>
                    <RXAxis dataKey="day" tick={{ fill: "hsl(215 20% 65%)", fontSize: 11 }} axisLine={false} tickLine={false} />
                    <RYAxis tick={{ fill: "hsl(215 20% 65%)", fontSize: 11 }} axisLine={false} tickLine={false} unit="h" width={30} domain={[0, "auto"]} />
                    <Tooltip contentStyle={{ background: "hsl(217 33% 17%)", border: "1px solid hsl(217 33% 22%)", borderRadius: "8px", color: "hsl(210 40% 96%)" }} />
      ...
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-primary text-primary-foreground flex items-center justify-center shadow-lg glow-lavender z-50"
      >
